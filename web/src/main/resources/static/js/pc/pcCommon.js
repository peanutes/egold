function contractUs() {
	layer.open({
		...
		area: ['420px', '240px'], //宽高
		shadeClose: true,
		content: '<div class="contractDiv">' +
		'<img src="../pcImages/user.png" width="58" height="58">' +
		'<div>9:00-21:00</div>' +
		'<div style="font-size:24px;">4000 328 128</div>' +
		'<div>邮箱：mail.egoldvip.com</div>' +
		'</div>'
	});
}
...
function downloadApp() {
	layer.open({
		type: 1,
		title: '扫描二维码下载手机版体验更多功能',
		skin: 'layui-layer-rim', //没有背景色
		//area: ['340px', '320px'], //宽高
		shadeClose: true,
		content: '<div style="text-align: center; width:100%;"><img style="text-align: center" src="../pcImages/erweima.png" width="285" height="285"></div>'
	});
}

//登出帐号
function logout() {
	$.post(getApiUrl("/member/member/logout"), {}, function (result) {
...
function goldTrendChart(chartObj, type, result) {
	...
	var dataDpr = 1, times, prices, max, min, option, interval = 'auto', width = $(window).width();
	times = result.times;
...
function getApiUrl(serviceAddress) {
	var queryString = systemConfig.queryString4Pc;
